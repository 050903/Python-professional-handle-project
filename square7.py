...
COLOR_TARGET_GREEN = (0, 255, 100) # Color for targeting system
...
pygame.display.set_caption("Project Starflight: Hyperspace Initiative v.2037") # Updated caption
clock = pygame.time.Clock()

# --- Sound Loading (Replace with your actual sound file paths) ---
try:
    engine_hum_sound = pygame.mixer.Sound("sounds/engine_hum.ogg") # Placeholder path
    warp_sound = pygame.mixer.Sound("sounds/warp_activate.ogg") # Placeholder path
    impact_sound = pygame.mixer.Sound("sounds/impact.ogg") # Placeholder path
    engine_hum_sound.set_volume(0.3) # Adjust volume
    warp_sound.set_volume(0.5)
    impact_sound.set_volume(0.7)
except pygame.error as e:
    print(f"Could not load sound: {e}. Sounds will be disabled.")
    engine_hum_sound = None
    warp_sound = None
    impact_sound = None
...
def project_point(point, camera_x, camera_y, camera_z, screen_center_x, screen_center_y, projection_distance=CAMERA_DEFAULT_Z):
    ...
    Returns (projected_x, projected_y, size_factor, original_z) or None if clipped."""
...
class Base3DObject:
    ...
    def draw(self, win, camera_pos, outline_color=None):
        """Draws the 3D object with back-face culling and Z-sorting.
        Optional outline_color for targeting."""
        center_x, center_y, center_z = camera_pos
        ...
        # Draw outline if specified (for targeting)
        if outline_color:
            for face_idx, face in enumerate(self.faces):
                projected_face_points = [projected_vertices_data[v_idx] for v_idx in face]
                if all(p is not None for p in projected_face_points):
                    points_2d = [(p[0], p[1]) for p in projected_face_points]
                    if len(points_2d) >= 2: # Lines need at least 2 points
                        # Draw lines connecting the vertices of the face
                        pygame.draw.lines(win, outline_color, True, points_2d, 2)
...
def main():
    ...
    while running:
        ...
        # Play engine hum if moving
        if engine_hum_sound:
            if current_speed > 0.1 and not pygame.mixer.Channel(0).get_busy():
                pygame.mixer.Channel(0).play(engine_hum_sound, -1) # Loop indefinitely
            elif current_speed <= 0.1 and pygame.mixer.Channel(0).get_busy():
        ...
        targeted_object = None
        target_distance = float('inf')
        current_camera_pos = (camera_x, camera_y, camera_z)

        # Check asteroids for closest target
        for asteroid in asteroids:
            dist = math.sqrt(
                (asteroid.x - current_camera_pos[0])**2 +
                (asteroid.y - current_camera_pos[1])**2 +
                (asteroid.z - current_camera_pos[2])**2
            )
            if dist < target_distance:
                target_distance = dist
                targeted_object = asteroid
        
        # Also consider the static station as a target
        for obj in objects:
            if isinstance(obj, ShipModel): # Assuming ShipModel is your 'station'
                 dist = math.sqrt(
                    (obj.x - current_camera_pos[0])**2 +
                    (obj.y - current_camera_pos[1])**2 +
                    (obj.z - current_camera_pos[2])**2
                )
                 if dist < target_distance:
                    target_distance = dist
                    targeted_object = obj
        ...
        for obj in objects + asteroids: # Combine static objects and dynamic asteroids
            ...
            # Determine if this object is the targeted one to draw an outline
            outline_color = None
            if obj is targeted_object:
                outline_color = COLOR_TARGET_GREEN
            ...
            for i, face in enumerate(obj.faces):
                if all(proj_data is not None for proj_data in [projected_vertices_data[v_idx] for v_idx in face]):
                    ...
                    all_polygons_to_draw.append((face_avg_z, color, points_2d, outline_color)) # Pass outline_color with face data
        
        # Sort polygons by their Z-depth for correct rendering
        all_polygons_to_draw.sort(key=lambda x: x[0], reverse=True) 

        for z_depth, color, points_2d, outline_color in all_polygons_to_draw:
            if len(points_2d) >= 3: 
                min_x = min(p[0] for p in points_2d)
                max_x = max(p[0] for p in points_2d)
                min_y = min(p[1] for p in points_2d)
            ...
            # Draw outline if present for this face
            if outline_color and len(points_2d) >= 2:
                pygame.draw.lines(win, outline_color, True, points_2d, 2)
        ...
        # Target info HUD
        if targeted_object:
            target_info_text = f"TARGET: {type(targeted_object).__name__.upper()} | DIST: {target_distance:.1f} U"
            target_info_render = font.render(target_info_text, True, COLOR_TARGET_GREEN)
            win.blit(target_info_render, (10, 70))
